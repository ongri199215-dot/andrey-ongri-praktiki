...
`;
css += `.practice-detail-card{width:min(440px,calc(100% - 40px));display:flex;min-height:0;flex-direction:column;gap:18px;padding:clamp(28px,5vw,58px) 0}.practice-detail-card .cover{width:100%}.actions-compact{width:100%;display:grid;grid-template-columns:1fr 1fr;margin:0;gap:10px}.actions-compact a{display:flex;align-items:center;justify-content:center;min-width:0;min-height:46px;padding:0 12px;border-radius:999px;font-size:9px;letter-spacing:.08em;white-space:nowrap}@media(max-width:650px){.practice-detail-card{width:min(360px,calc(100% - 36px));display:flex;padding:20px 0}.practice-detail-card .cover{max-width:none}.actions-compact a{padding:0 8px}}`;
...
const detail = page({
  body: `<main class="showcase detail-page"><header class="topbar detail-topbar"><span>ПРАКТИКИ ОТ АНДРЕЯ ОНГРИ</span><a href="../../">← ВСЕ ПРАКТИКИ</a></header><section class="detail-card practice-detail-card"><a class="cover" href="read/"><img src="../../${cover}" alt="Первая страница гайда ${title}"></a><div class="actions actions-compact"><a href="read/">Смотреть практику</a><a class="download-guide" href="../../${guide}" download>Скачать</a></div></section></main>`,
});
